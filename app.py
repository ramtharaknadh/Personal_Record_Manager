...
with mysql.connector.connect(host='localhost',user='root',password='admin',db='prm') as conn:
    ...
    cursor.execute('create table if not exists files(fid binary(16) primary key,filetitle varchar(50),extension varchar(8),filedata longblob,date timestamp default now() on update now(),added_by varchar(15), FOREIGN KEY (added_by) REFERENCES users(username))')
    ...
    if session.get('user'):
        if request.method=='POST':
            ...
            for file in files:
                ...
                cursor.execute('insert into files(fid,filetitle,extension,filedata,added_by) values(uuid_to_bin(uuid()),%s,%s,%s,%s)',[filetitle,file_ext,file_data,username])
                mydb.commit()
            cursor.close()
...
def filesview():
    if session.get('user'):
        username=session.get('user')
        cursor=mydb.cursor(buffered=True)
        cursor.execute('select bin_to_uuid(fid) as uid,filetitle,date from files where added_by=%s order by date desc',[username])
        data=cursor.fetchall()
...
@app.route('/fileupdate/<uid>', methods=['GET', 'POST'])
def fileupdate(uid):
    if session.get('user'):
        if request.method == 'POST':
            filetitle=request.form['filetitle']
            ...
            username = session.get('user')
            cursor = mydb.cursor(buffered=True)
            file_ext = new_file.filename.split('.')[-1]
            file_data = new_file.read()  # reads binary data from file
            cursor.execute('update files set filetitle=%s,extension=%s, filedata=%s where bin_to_uuid(fid)=%s and added_by=%s',
                           [filetitle,file_ext, file_data, uid, username])
            mydb.commit()
            cursor.close()
            flash('File updated successfully')
            return redirect(url_for('filesview'))
        else:
            # This is the GET request for rendering the form to update file
            cursor = mydb.cursor(buffered=True)
            cursor.execute('select filetitle,extension, filedata from files where bin_to_uuid(fid)=%s', [uid])
            filetitle,ext, bin_data = cursor.fetchone()
            cursor.close()
            return render_template('fileupdate.html', uid=uid,filetitle=filetitle, ext=ext)
    else:
        return redirect(url_for('login'))
